#' @export
makeRLearner.regr.kerasff = function() {
  makeRLearnerRegr(
    cl = "regr.kerasff",
    package = "keras",
...
#' @export
trainLearner.regr.kerasff  = function(.learner, .task, .subset, .weights = NULL,
  ...
  rho = 0.9, loss = "mean_squared_error", batch_size = 128L, layers = 1,
  ...
  act_layer = "relu", l1_reg_layer = 0.001, l2_reg_layer = 0.001, validation_split = 0.2) {
  ...
  model = model %>% layer_dense(units = 1L, activation = "linear")
...
#' @export
predictLearner.regr.kerasff = function(.learner, .model, .newdata, ...) {
