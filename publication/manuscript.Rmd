...
They present a survey of indicator-based and embedding-based methods but do not conduct a benchmark study.
...
Throughout our analysis, we use implementations of the various methods for the open-source statistical software R [@r_2021]. To enable a fair and reliable comparison in our study, we implemented all encoding methods on top of the `mlrCPO` package [@binder_2018].
...
Methods that can jointly encode several variables and therefore leverage correlation structures between the different variables are an interesting avenue of research in the future:
...
Those should be compared to classical approaches from the optimal scaling literature [e.g. @deleeuw_1976] leading to the *aspect* framework [@mair_2010], which is unfortunately unfamiliar to many ML researchers.
