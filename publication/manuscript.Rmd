...
set.seed(42)
knitr::opts_chunk$set(cache.extra = knitr::rand_seed, echo = FALSE, fig.path = "", dev=c('pdf', 'tiff'))
...
Although domain knowledge can sometimes be used to reduce the number of theoretically relevant levels, finding strategies that work well on a large variety of problems is highly important for many applications as well as in automated ML [@thomas_2018; @thornton_2013; @feurer_2015]. Optimally, strategies should be model-agnostic because benchmarking encoding methods together with ML algorithms from different classes is often necessary for applications.
...
Furthermore, there has recently been increasing attention on scientific benchmark studies that compare different methods to provide a clearer picture in light of a large number of methods available to practitioners [@delgado; @bommert], as they can provide at least partial answers to such questions.
...
Following calls in the computational statistics community for neutral benchmark studies [@boulesteix_2017], which do not introduce a new method, thus reducing the risk of cherry picking methods [@dehghani2021benchmark] and reporting over-optimistic performance [@niesl2021overoptimism], we present a carefully designed experimental setting to discern the effect of encoding strategies and their interaction with different ML algorithms.
...
## Categorical Encoding Benchmarks
...
A recent study ($15$ datasets) found good performance of target encoding, but they only investigated categorical features in regression settings [@seca_2021]. 
...
As constructing all indicator variables is practically infeasible for high cardinality variables, we limit their number by collapsing rare levels beyond a varying threshold to a single *other* category before encoding.
...
In regression, the conditional modes are similar to the mean target value for each level, weighted by the relative observed frequency of that level in the training set [@gelman_2006; @bates_2018].
...
Datasets contain between `r descr_dat[, min(sapply(strsplit(HighCardLevels, " "), length))]` and `r descr_dat[, max(sapply(strsplit(HighCardLevels, " "), length))]` categorical features with more than $10$ levels, with the maximum number of levels for a feature ranging between `r descr_dat[, min(MaxCatLvls)]` and `r descr_dat[, max(MaxCatLvls)]`. Missing values are present in about half of the datasets.
...
GB models were trained using `xgboost` [@chen_2018], setting the learning rate to $0.01$ and determining the number of iterations using early stopping on a 20% holdout set.
KNN was taken from package `kknn` [@schliep_2016] standardizing features and using a constant $k=15$ for the number of nearest neighbors, together with an information gain filter [@brown2012] to limit the number of features to $25$.
...
**Final one-hot encoding**: Transform all remaining
...
```{r datasets, warning=FALSE, results="hide", fig.pos="h"}
library(kableExtra)
landscape(
  knitr::kable(
...
All materials for this study, including reproducible code for this manuscript and result objects can be downloaded from our **online repository** \footnote{\url{https://github.com/slds-lmu/paper_2021_categorical_feature_encodings}}.
...
Performance in some CV folds was below the FL learner for *flight-delay-usa-dec-2017* ($RMSE_{FL} =$ `r round(res[lrn.id == "FL" & Name == "flight-delay-usa-dec-2017", rmse.test.mean], 2)`) and *nyc-taxi-green-dec-2016* ($RMSE_{FL} =$ `r round(res[lrn.id == "FL" & Name == "nyc-taxi-green-dec-2016", rmse.test.mean], 2)`).
...
To further analyze our results, we used statistical inference methods inspired by the benchmark community in computational statistics [e.g., @hothorn_2005]. 
First, we present meta rankings for each ML algorithm in Figure \ref{fig:rank-res}:
We defined an encoder relation within each dataset, based on corrected resample t-tests [@nadeau_2003].
...
(ref:rank-res) Consensus rankings across all datasets for each algorithm. Lower ranks indicate better performance. The rank of the *none* control condition of the RF (rank 11 of 12) was omitted from the figure.
...
We found that one-hot encoding usually gave a slightly better performance than dummy encoding, which has also been observed by @chiquet_2016 and @tutz_2016 (p.254).
...
Recent approaches use target-based encoding strategies to learn a vector-valued representation of a level, similar to neural network embeddings [@guo_2016;@rodriguez_2018].
Those should be compared to classical approaches from the optimal scaling literature [e.g. @deleeuw_1976;@young_1976] or the subsequent *aspect* framework [@mair_2010], which are unfortunately unfamiliar to many ML researchers.
